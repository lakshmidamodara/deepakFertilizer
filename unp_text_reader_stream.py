...
global wbs_start_line_number, wbs_end_lineno
wbs_start_line_number = 0
wbs_end_lineno = 0
...
result = sys.stdin.read().encode('utf-8',errors='ignore').strip()
print(result,file=actvity_txtFile)
str = result.decode('utf-8').split("\n")
#print((str))

def getStartingLineNumber(identifier):
    try:
        global wbs_start_line_number
        #get the wbs_id starting linenumber
        for i, line in enumerate(str):
            if identifier in line:
                wbs_start_line_number = i
                break
        return wbs_start_line_number
    except (ErrorOccured) as error:
        print('Error occurrd in getStartingLineNumber():', error)
        print(sys.exc_traceback.tb_lineno)

def getEndingLineNumber(identifier):
    try:
        global wbs_end_lineno
        #get the wbs-id ending line number
        print(wbs_start_line_number)
        for i in range(0, len(str)):
             if identifier in str[i]:
                if i > wbs_start_line_number:
                    wbs_end_lineno = i
                    break

        return wbs_end_lineno
    except (ErrorOccured) as error:
        print('Error occurrd in getEndingLineNumber():', error)
        print(sys.exc_traceback.tb_lineno)
...
def ReadWriteTable(identifier,start_identifier,end_identifier):
    try:
        ...
        wbs_start_line_number = getStartingLineNumber(start_identifier)
        wbs_end_lineno = getEndingLineNumber(end_identifier)
        # now write the wbs_id data into a file with starting and ending line numbers
        line = str[wbs_start_line_number+1:wbs_end_lineno]
