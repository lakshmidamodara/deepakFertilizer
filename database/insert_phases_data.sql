...
CREATE OR REPLACE FUNCTION insert_phases_data(p_name text, p_sch_start date, p_sch_end date, p_act_start date, p_act_end date, p_project_id integer,
												p_planned_start date, p_planned_end date)
RETURNS INTEGER AS $$
DECLARE
	l_phase_id INTEGER;
BEGIN
	...
	WHERE
		...
		AND p.planned_start IS NOT DISTINCT FROM p_planned_start
		AND p.planned_end IS NOT DISTINCT FROM p_planned_end;
    
	RETURN l_phase_id;
	
	EXCEPTION
		WHEN NO_DATA_FOUND THEN
			INSERT INTO public.phases (name, scheduled_start, scheduled_end, actual_start, actual_end, project_id, planned_start, planned_end) 
			VALUES (p_name, p_sch_start, p_sch_end, p_act_start, p_act_end, p_project_id, p_planned_start, p_planned_end);
