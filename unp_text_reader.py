...
def getTableStartAndEndPositions(table_id):
    ...
    with open(infile, 'r', errors='ignore') as f:
        ...
        reader = f.read().encode("utf-8").strip()
        str = reader.decode('utf-8').split("\n")
...
def ReadWriteWBSID():
    try:
        ...
        with open(infile, 'r', errors='ignore') as f:
            reader = f.read().encode("utf-8").strip()
            str = reader.decode('utf-8').split("\n")
            for line in str[wbs_tab_start:wbs_tab_end-1]:
                row.append([line])
...
def ReadWriteTaskPredecessor():
    try:
        ...
        with open(infile, 'r', errors='ignore') as f:
            ...
            # read the lines from the xer file line by line
            # starting from the locations obtained above
            for line in str[task_predecessor_start:task_predecessor_end-1]:
                row.append([line])
...
def ReadwriteTASKS():
    try:
        ...
        with open(infile, 'r', errors='ignore') as f:
            ...
            for line in str[task_tab_start:task_tab_end-1]:
